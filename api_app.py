...
async def validation_exception_handler(request, exc: RequestValidationError):
    ...
    try:
        logger.warning(
            'event=validation_error detail="%s"',
            detail,
        )
    except Exception:
        pass
...
def metrics():
    ...
    avg_latency = (
        _metrics["total_latency_ms"] / total
        if total > 0
        else 0.0
    )

    low_conf_rate = (
        _metrics["low_confidence_count"] / total
        if total > 0
        else 0.0
    )
...
def predict(request: PredictRequest):
    ...
    try:
        ...
        try:
            logger.error(
                'event=server_error request_id=%s detail="%s"',
                request_id,
                str(e),
            )
        except Exception:
        ...
        try:
            logger.error(
                'event=prediction_error request_id=%s detail="%s"',
                request_id,
                str(e),
            )
        except Exception:
    ...
    try:
        logger.info(
            "event=prediction_request request_id=%s text_length=%s "
            "label=%s confidence=%s low_confidence=%s latency_ms=%s",
            request_id,
            len(request.text),
            result["label"],
            result["confidence"],
